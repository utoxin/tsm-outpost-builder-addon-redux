...
MOD_NAME = "tsm-outpost-builder-redux"
...
function update_me_combo()
    --
    if table_size(global.me_combinators) == 0 then
        return
    end
    ...
    for _, combo in pairs(global.me_combinators) do
        local ccb = combo.entity.get_or_create_control_behavior()
        ccb.parameters = nil
        for index, signal in pairs(green.signals) do
            --    signal = {signal={type="item", name=item},count=count}
            --   game.print(signal.signal.name)
...
local function exclusions_shortcut(event)
    ...
    if prototype ~= "TSM-OB-exclusion" then
        return
    end
...
local function on_gui_elem_changed(event)
    ...
    if mod ~= "tsm-outpost-builder-redux" then
        return
    end
...
function pop_all_green()
    global.green = global.green or {}
    if table_size(global.green) == 0 then
        return
    end
...
local function on_gui_click(event)
    ...
    elseif string.find(element.name, "ex_remove") then
        ...
        for i, player in pairs(game.players) do
            if player.gui.screen.exclusion_frame ~= nil then
